...
def test_juice_shop_complete_behavioral_equivalence(
    ...
    try:
        ...
        pre_rebuild_account = f"vdy-rebuild-{original_run[:12]}@example.test"
        _register_and_login(port, pre_rebuild_account)
        ...
        login_status, _ = _request(
            ...
            "/rest/user/login",
            method="POST",
            payload={"email": pre_rebuild_account, "password": "Vdy-Smoke-1!"},
        )
