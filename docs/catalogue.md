...
|---|---|---|
| OWASP Juice Shop 20.2.0 | runnable | `upstream-pinned`; official release automation, immutable multi-arch index with amd64 verified, GitHub-hosted amd64 identity/challenge/lifecycle smoke passed with the Docker 28 minimum enforced; upstream force-recreates its SQLite schema on each process start, so application data is explicitly ephemeral; arm64 and an image signature are not verified. |
