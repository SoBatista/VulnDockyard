...
def _residual() -> dict[str, list[str]]:
    ...
    for kind, command in {
        "containers": ("docker", "container", "ls", "--all", "--quiet", "--filter"),
        "networks": ("docker", "network", "ls", "--quiet", "--filter"),
        "volumes": ("docker", "volume", "ls", "--quiet", "--filter"),
    }.items():
        query = subprocess.run(  # noqa: S603 - fixed read-only Docker audit command
            (*command, "label=org.vulndockyard.managed=true"),
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
        ...
        result[kind] = sorted(line for line in query.stdout.splitlines() if line)
    return result
...
def _phases() -> tuple[Phase, ...]:
    ...
    return (
        ...
        Phase("static-typing", (python, "-m", "mypy", "src", "tests"), 180),
        Phase(
        ...
        Phase("docker-preflight", ("docker", "info", "--format", "{{json .ServerVersion}}"), 30),
        Phase(
        ...
        "remote_bootstrap_gates": [
            {
                ...
                "result": "not-run",
                "reason": "requires a pushed commit on GitHub-hosted runners",
            },
            {
                "gate": "repository ruleset and private vulnerability reporting",
                "result": "not-run",
                "reason": "requires repository settings after the first push",
            },
            {
